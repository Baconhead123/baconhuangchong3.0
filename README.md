...
do
	...
	-- Real torso accessory loader.
	-- Asset 70523944235011 is loaded by catalog ID only and is welded directly
	-- to the original gray RigTorso block. The gray block remains at 0.75
	-- transparency; the accessory itself remains visible.
	local TorsoAccessoryId = 70523944235011
	local TorsoRigTransparency = 0.75

	local LoadTorsoAccessory = function(AssetId)
		local Loaded, Asset = pcall(function()
			return Services.InsertService:LoadAsset(tonumber(AssetId))
		end)

		if not Loaded or not Asset then
			Loaded, Asset = pcall(function()
		...
		local Accessory = nil
		if IsA(Asset, "Accoutrement") then
			Accessory = Asset
		else
			Accessory = FindFirstChildOfClass(Asset, "Accoutrement")
			if not Accessory then
				for _, Child in next, GetDescendants(Asset) do
		...
		local Handle = FindFirstChildOfClassAndName(Accessory, "BasePart", "Handle")
		if not Handle then
			pcall(Destroy, Asset)
			return nil
		end

		-- This is the original gray torso block. Do NOT replace or destroy it.
		local RigTorso = FindFirstChildOfClassAndName(Rig, "BasePart", "RigTorso")
		if not RigTorso then
			pcall(Destroy, Asset)
			return nil
		end

		-- Keep the gray RigTorso visible at the requested transparency.
		RigTorso.Transparency = TorsoRigTransparency
		RigTorso.CanCollide = false
		RigTorso.CanTouch = false
		RigTorso.CanQuery = false
		RigTorso.Massless = true

		-- Remove an older copy of this catalog accessory only.
		for _, Child in next, GetDescendants(Rig) do
			if Child.Name == "TorsoAccessory70523944235011" and Child ~= RigTorso then
				pcall(Destroy, Child)
			end
		end

		local HandleClone = Clone(Handle)
		HandleClone.Name = "TorsoAccessory70523944235011"
		HandleClone.Anchored = false
		HandleClone.CanCollide = false
		HandleClone.CanTouch = false
		HandleClone.CanQuery = false
		HandleClone.Massless = true
		HandleClone.Transparency = 0

		-- Remove old joints so only our torso weld controls the accessory.
		for _, Descendant in next, GetDescendants(HandleClone) do
			if IsA(Descendant, "JointInstance") then
				pcall(Destroy, Descendant)
			end
		end

		HandleClone.Parent = Rig

		-- Bind the accessory to the actual gray RigTorso block.
		-- Start from the exact block position; no HumanoidRootPart/UpperTorso
		-- fallback and no MeshId/TextureId matching is used here.
		-- Applied a fixed Z rotation of 60 degrees to the accessory handle.
		HandleClone.CFrame = RigTorso.CFrame * CFrame.Angles(0, 0, math.rad(60))
	...
	Start = function(Options)
		if not IsRunning then
			...
			local LoadedTorso = LoadTorsoAccessory(TorsoAccessoryId)
			if not LoadedTorso then
				pcall(function()
					StarterGui:SetCore("SendNotification", {
						Title = "Cherry's Club",
						Text = "躯干饰品 " .. tostring(TorsoAccessoryId) .. " 加载失败，已保留原躯干。",
						Duration = 5,
					})
				end)
			end
			...
			for Index, Descendant in next, GetDescendants(Rig) do
				...
				elseif IsA(Descendant, "BasePart") or IsA(Descendant, "Decal") then
					if Descendant.Name ~= "RigTorso" then
...
Empyrean = Start({
	Accessories = {
		-- Swords / Spears Rig
		{ Name = "RigTorso", AccessoryId = 70523944235011, MeshId = "92048094600083",  TextureId = "89519907183882", Offset = CFrame.new(0, -2, 0) * CFrame.Angles(0, Rad(0), Rad(60)) },
		...
		-- Block Rig
		{ Name = "RigTorso", AccessoryId = 70523944235011, MeshId = "17345242336",  TextureId = "17345250641", Offset = CFrame.new(0, -2, 0) * CFrame.Angles(0, Rad(0), Rad(60)) },
